	...
	public class OmokMouseListener extends MouseAdapter {
		...
		public void mouseClicked(MouseEvent e) {
			...
			for(int i=2;i<line+1;i++){
				...
				if(checkFive(player)){
						JOptionPane.showMessageDialog(new OmokGamePage_client(), "흰색의 승리", "게임 끝", JOptionPane.INFORMATION_MESSAGE);
						System.exit(0);
				}
		...
		while(true){
			try {
				...
				if(checkFive(dto.getPlayer())){
					JOptionPane.showMessageDialog(new OmokGamePage_client(), "검은색의 승리", "게임 끝", JOptionPane.INFORMATION_MESSAGE);
					System.exit(0);
				}
				pan.addMouseListener(om);
			//	break;

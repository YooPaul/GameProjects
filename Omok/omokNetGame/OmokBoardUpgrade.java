...
public class OmokBoardUpgrade extends JPanel  {
	...
	private int xpos;
	private int ypos;
	public static OmokBoardUpgrade ob;
		...
		for(int i=2;i<line+1;i++){
			...
			if(checkFive()){
				if(turn==1){
					...
					JOptionPane.showMessageDialog(OmokBoardUpgrade.ob, "White Win", "Game Over", JOptionPane.INFORMATION_MESSAGE);
				//System.exit(0);
			}
			return true;
			}else
	...
	public static void main(String[] args){
		...
		f.setResizable(false);
		OmokBoardUpgrade.ob=new OmokBoardUpgrade();
		f.setLayout(null);
		JPanel pan=new JPanel();
		JPanel msgPan=new JPanel();
		msgPan.setLayout(null);
		msgPan.setBounds(0,0,938-ob.getWidth(),600);
		msgPan.setBackground(new Color(66, 197, 244));
		JTextArea ta2=new JTextArea();
		ta2.setBounds(0,600,270,70);
		ta2.setFont(new Font("Arial", Font.BOLD, 14));
		setAction(msgPan,ta2);
		pan.setBounds(ob.getWidth(), 0, 938-ob.getWidth(), 660);
		pan.setLayout(null);
	...
	private static class ReceiveMessage extends Thread{
		...
		private JPanel pan;
		public ReceiveMessage(DataInputStream dis, JPanel pan){
			this.dis=dis;
			this.pan=pan;
		}
		public void run(){
			...
			while(dis!=null){
				try {
					msg=dis.readUTF();
					MessageBox mb=new MessageBox(msg,1);
					mb.setBounds(10,msgPosy,mb.getWidth(),mb.getHeight());
					msgPosy+=mb.getHeight()+10;
					pan.add(mb);
					pan.repaint();
				} catch (IOException e) {
					// TODO Auto-generated catch block
	...
	private static void setAction(JPanel pan,JTextArea ta2){
			...
			public void keyPressed(KeyEvent arg0) {
				...
				if(arg0.getKeyCode()==KeyEvent.VK_ENTER){
					...
					try {
						...
						MessageBox mb=new MessageBox(msg,0);
						mb.setBounds(pan.getWidth()-10-mb.getWidth(),msgPosy,mb.getWidth(),mb.getHeight());
						msgPosy+=mb.getHeight()+10;
						pan.add(mb);
						pan.repaint();
					} catch (IOException e) {
		...
		new ReceiveMessage(chatdis,pan).start();
	}
